...
import { CurrencyDisplayContext } from './SnapshotFormat'
...
export function ComparisonPanel({current,baseline,onBaseline,disabled,refreshKey,autoCalculate}:Props) {
  const currencyDisplay = useContext(CurrencyDisplayContext)
  const displayText = (text:string) => snapshotText(text, current.provenance, currencyDisplay)
  ...
  return <section aria-label="Scenario comparison" className="bg-surface-lighter rounded-xl p-4 sm:p-5 space-y-4">
    <div className="flex justify-between items-start gap-4">
      <div><h3 className="font-semibold">Compare scenarios</h3><p className="text-sm text-text-secondary mt-1">{autoCalculate ? 'Pin this result, then change your inputs to compare automatically. Or choose a saved result.' : 'Pin this result, change your inputs, then calculate again. Or choose a saved result.'}</p></div>
